...
class ode_solve
{
    ...
    public:
        ...
        std::vector<Vector> impliedRK2(int );
 };
...
std::vector<Vector> ode_solve::Kutta3(int n){
    int m=yInitial.size();
    double dx=(xEnd-xInitial)/double(n);
    std::vector<Vector> y ;
    y.resize(n+1);
    ...
    for(int k=0;k<n;k++){
        double xTemp=xInitial+k*dx;
        Vector K1(m,0),K2(m,0),K3(m,0);
        y[k+1].resize(m);
        K1=sfn(xTemp,y[k]);
        K2=sfn(xTemp+0.5*dx,y[k]+0.5*dx*K1);
        K3=sfn(xTemp+dx,y[k]-dx*K1+2*dx*K2);
        y[k+1]=y[k]+dx/6.0*(K1+4*K2+K3);
    }
    return y;
...
std::vector<Vector> ode_solve::impliedRK2(int n){
    int m=yInitial.size();
    double dx=(xEnd-xInitial)/double(n);
    std::vector<Vector> y;y.resize(n+1);
    y[0].resize(m);
    y[0]=yInitial;
    for(int k=0;k<n;k++){
        double tp=xInitial+dx*k;
